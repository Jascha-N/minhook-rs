...
// Pointer for calling original NtAllocateVirtualMemory.
...
NTSTATUS NTAPI DetourNtAllocateVirtualMemory(HANDLE ProcessHandle, PVOID *BaseAddress, ULONG ZeroBits, PULONG RegionSize, ULONG AllocationType, ULONG Protect)
{
    ...
    return fpNtAllocateVirtualMemoryOrig(ProcessHandle, BaseAddress, ZeroBits, RegionSize, AllocationType, Protect);
}
...
BOOL WINAPI DllMain(_In_ HINSTANCE hinstDLL, _In_ DWORD fdwReason, _In_ LPVOID lpvReserved)
{
    ...
    case DLL_PROCESS_ATTACH:
        ...
        fpNtAllocateVirtualMemory = (NTALLOCATEVIRTUALMEMORY)(MyGetProcAddress(ntdll, "NtAllocateVirtualMemory"));
        ...
        if (MH_CreateHook(fpNtAllocateVirtualMemory, &DetourNtAllocateVirtualMemory,
            (LPVOID*)(&fpNtAllocateVirtualMemoryOrig)) != MH_OK)
        {
            return FALSE;
        }

        // Enable the hook.
        if (MH_EnableHook(fpNtAllocateVirtualMemory) != MH_OK)
        {
            return FALSE;
        }
    }
